...
//   // }, [tasks]);
// // Check for overdue tasks safely
// useEffect(() => {
//   const checkOverdueTasks = () => {
//     const now = new Date();
//     let triggeredAlert = false;
...
//     tasks.forEach(task => {
//       // CRITICAL: Ensure task.date exists and is NOT a blank string before parsing
//       if (task.date && task.date.trim() !== '' && !task.completed) {
//         const taskDate = new Date(task.date);
        
//         // Double check it's a valid date object to prevent crashes
...
if ("Notification" in window && Notification.permission === "granted") {
  new Notification("Tick App: Task Overdue!", {
    body: `"${task.text}" is past its due date!`,
    icon: "/favicon.svg" // <-- This adds your logo icon back to the notification popup
  });
}
          }
        }
  ...
  const handleSaveEdit = (updatedTask) => {
    setTasks(tasks.map(t => t.id === updatedTask.id ? updatedTask : t));
    alertedTasksRef.current.delete(updatedTask.id);
    setEditingTask(null);
  };
  ...
  // FIXED: Added safe verification wrappers to block invalid date objects from causing rendering whiteout crashes
  const hasOverdue = tasks.some(t => {
    if (!t.date || t.date.trim() === '' || t.completed) return false;
  ...
  return (
    <div className="h-screen w-full flex flex-col bg-[var(--bg-primary)] overflow-hidden select-none">
      <Header 
        ...
        doneCount={doneCount} 
        isRinging={hasOverdue} 
        isDarkTheme={isDarkTheme} // Pass state prop down
        toggleTheme={() => setIsDarkTheme(!isDarkTheme)} 
      />
